...
#ifndef ETHARD_H_INCLUDED
#define ETHARD_H_INCLUDED
...
#define ETHARD_VERSION_MAJOR 0
#define ETHARD_VERSION_MINOR 0

/// The version number of the Cyphal specification implemented by this library.
#define ETHARD_CYPHAL_SPECIFICATION_VERSION_MAJOR 1
#define ETHARD_CYPHAL_SPECIFICATION_VERSION_MINOR 0
...
#define ETHARD_ERROR_INVALID_ARGUMENT 2
#define ETHARD_ERROR_OUT_OF_MEMORY 3

/// In the case that we still need error codes but need to mutate an input we will default to a success code
#define ETHARD_SUCCESS 0

/// MTU values for the supported protocols.
#define ETHARD_MTU_UDP_IPV4 576U
#define ETHARD_MTU_UDP_IPV6 1280U
#define ETHARD_MTU_MAX ETHARD_MTU_UDP_IPV6  /// We may want to set this to 1400/1500 for Ethernet MTU

/// Parameter ranges are inclusive; the lower bound is zero for all. See Cyphal/UDP Specification for background.
#define ETHARD_SUBJECT_ID_MAX 8191U   /// 13 bits subject ID
#define ETHARD_SERVICE_ID_MAX 65535U  /// The hard limit for ports
#define ETHARD_NODE_SUBNET_MAX 128U   /// 7 bits for subnet
// #define ETHARD_NODE_ID_MAX 65535U     /// 16 bits is the hard limit. But this may change pending implementations
#define ETHARD_NODE_ID_MAX 255U  /// Setting to a lower value until we can determine a way to pre-allocate sessions
#define ETHARD_PRIORITY_MAX 7U
#define ETHARD_TRANSFER_ID_BIT_LENGTH 63ULL
#define ETHARD_TRANSFER_ID_MAX ((1ULL << ETHARD_TRANSFER_ID_BIT_LENGTH) - 1ULL)

#define ETHARD_NODE_ID_UNSET 0U  /// For UDP No ID is the anonymous ID
...
#define ETHARD_DEFAULT_TRANSFER_ID_TIMEOUT_USEC 2000000UL  /// 2 seconds

// Forward declarations.
typedef struct EthardInstance    EthardInstance;
typedef struct EthardTreeNode    EthardTreeNode;
typedef struct EthardTxQueueItem EthardTxQueueItem;
typedef uint64_t                 EthardMicrosecond;
typedef uint32_t                 EthardNodeAddress;  /// Full Node IP Address for multi or uni cast
typedef uint32_t                 EthardIPv4Addr;     /// Full Node IP Address for multi or uni cast
typedef uint16_t                 EthardPortID;
typedef uint16_t                 EthardUdpPortID;
typedef uint16_t                 EthardNodeID;
typedef uint64_t                 EthardTransferID;  /// TODO - This may need to be broken up for 32bit systems?

/// Transfer priority level mnemonics per the recommendations given in the Cyphal Specification.
typedef enum
{
    EthardPriorityExceptional = 0,
    EthardPriorityImmediate   = 1,
    EthardPriorityFast        = 2,
    EthardPriorityHigh        = 3,
    EthardPriorityNominal     = 4,  ///< Nominal priority level should be the default.
    EthardPriorityLow         = 5,
    EthardPrioritySlow        = 6,
    EthardPriorityOptional    = 7,
} EthardPriority;

/// Transfer kinds as defined by the Cyphal Specification.
typedef enum
{
    EthardTransferKindMessage  = 0,  ///< Multicast, from publisher to all subscribers.
    EthardTransferKindResponse = 1,  ///< Point-to-point, from server to client.
    EthardTransferKindRequest  = 2,  ///< Point-to-point, from client to server.
} EthardTransferKind;
#define ETHARD_NUM_TRANSFER_KINDS 3
...
struct EthardTreeNode
{
    EthardTreeNode* up;     ///< Do not access this field.
    EthardTreeNode* lr[2];  ///< Left and right children of this node may be accessed for tree traversal.
...
typedef struct
{
    EthardFrameHeader udp_cyphal_header;
    size_t            payload_size;
    const void*       payload;
} EthardFrame;
...
typedef struct
{
    ...
    EthardIPv4Addr source_route_specifier;
    EthardIPv4Addr destination_route_specifier;
...
typedef struct
{
    ...
    /// For outgoing message transfers the value shall be ETHARD_NODE_ID_UNSET (otherwise the state is invalid).
    ...
    /// For incoming anonymous transfers the value is reported as ETHARD_NODE_ID_UNSET.
    EthardNodeID remote_node_id;
...
/// API functions that work with this type are named "ethardTx*()", find them below.
typedef struct EthardTxQueue
{
    ...
    /// otherwise, networking interoperability issues may arise. See recommended values ETHARD_MTU_*.
    ///
...
struct EthardTxQueueItem
{
    ...
    /// remote nodes anyway, so all its remaining frames can be dropped from the queue at once using ethardTxPop().
    EthardTxQueueItem* next_in_transfer;

    /// This is the same value that is passed to ethardTxPush().
    /// Frames whose transmission deadline is in the past shall be dropped.
    EthardMicrosecond tx_deadline_usec;
...
typedef struct EthardRxSubscription
{
    EthardTreeNode base;  ///< Read-only DO NOT MODIFY THIS

    EthardMicrosecond transfer_id_timeout_usec;
    size_t            extent;   ///< Read-only DO NOT MODIFY THIS
    EthardPortID      port_id;  ///< Read-only DO NOT MODIFY THIS

    /// This field can be arbitrarily mutated by the user. It is never accessed by the library.
    /// Its purpose is to simplify integration with OOP interfaces.
    void* user_reference;

    /// TODO - We may need to limit ETHARD_NODE_ID_MAX so that we aren't allocating 65 KiB for empty sessions...
    ...
    struct EthardInternalRxSession* sessions[ETHARD_NODE_ID_MAX + 1U];  ///< Read-only DO NOT MODIFY THIS
} EthardRxSubscription;

/// Reassembled incoming transfer returned by ethardRxAccept().
typedef struct EthardRxTransfer
{
    EthardTransferMetadata metadata;
...
typedef void* (*EthardMemoryAllocate)(EthardInstance* ins, size_t amount);
...
///     - The execution time should be constant (O(1)).
typedef void (*EthardMemoryFree)(EthardInstance* ins, void* pointer);

/// This is the core structure that keeps all of the states and allocated resources of the library instance.
struct EthardInstance
{
    ...
    /// Invalid values are treated as ETHARD_NODE_ID_UNSET. The default value is ETHARD_NODE_ID_UNSET.
    EthardNodeID   node_id;
    EthardIPv4Addr local_ip_addr;
    ...
    ///
    /// The following API functions may allocate memory:   ethardRxAccept(), ethardTxPush().
    /// The following API functions may deallocate memory: ethardRxAccept(), ethardRxSubscribe(), ethardRxUnsubscribe().
    /// The exact memory requirement and usage model is specified for each function in its documentation.
    EthardMemoryAllocate memory_allocate;
    EthardMemoryFree     memory_free;

    /// Read-only DO NOT MODIFY THIS
    EthardTreeNode* rx_subscriptions[ETHARD_NUM_TRANSFER_KINDS];
};

/// TODO - Make a UDP acceptance filter based on the EthardFrameHeader
...
/// from the transmission queue using the function ethardTxPeek() and transmit them. Each transmitted (or otherwise
/// discarded, e.g., due to timeout) frame should be removed from the queue using ethardTxPop(). The queue is
...
///     - The remote node-ID is not ETHARD_NODE_ID_UNSET and the transfer is a message transfer.
///     - The remote node-ID is above ETHARD_NODE_ID_MAX and the transfer is a service transfer.
...
/// frames of serialized transfers pushed into the prioritized transmission queue by ethardTxPush().
///
/// The timestamp values of returned frames are initialized with tx_deadline_usec from ethardTxPush().
...
/// application by calling ethardInstance::memory_free(). The memory shall not be freed before the entry is removed
/// from the queue by calling ethardTxPop(); this is because until ethardTxPop() is executed, the library retains
/// ownership of the object. The pointer retains validity until explicitly freed by the application; in other words,
/// calling ethardTxPop() does not invalidate the object.
///
...
///
/// Upon return, the out_subscription pointer will point to the instance of ethardRxSubscription that accepted this
...
/// The purpose of this argument is to allow integration with OOP adapters built on top of libethard; see also the
/// user_reference provided in ethardRxSubscription.
///
...
///        ethardRxUnsubscribe(). The number of sessions is bounded and the bound is low (at most the number of nodes
...
///        The amount of the allocated memory equals the extent as configured via ethardRxSubscribe(); please read
...
/// nodes emitting transfers that match the subscription (which cannot exceed (ETHARD_NODE_ID_MAX-1) by design).
...
///
/// If such subscription already exists, it will be removed first as if ethardRxUnsubscribe() was
...
///
/// The default transfer-ID timeout value is defined as ETHARD_DEFAULT_TRANSFER_ID_TIMEOUT_USEC; use it if not sure.
...
/// existed; the deallocation behavior is specified in the documentation for ethardRxUnsubscribe().
///
...
/// This function reverses the effect of ethardRxSubscribe().
/// If the subscription is found, all its memory is de-allocated (session states and payload buffers); to determine
/// the amount of memory freed, please refer to the memory allocation requirement model of ethardRxAccept().
///
